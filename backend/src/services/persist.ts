...
export function persistBootstrap(db: Database.Database, data: BootstrapPayload): void {
  const tx = db.transaction(() => {
    ...
    for (const l of data.leads) insertLead.run(l.id, l.name, l.company, l.email, l.stage, l.value, l.lastContact);

    const insertInvoice = db.prepare(
      'INSERT INTO invoices (id, client_name, amount, status, due_date) VALUES (?, ?, ?, ?, ?)'
    );
    for (const i of data.invoices) insertInvoice.run(i.id, i.clientName, i.amount, i.status, i.dueDate);

    const insertNotif = db.prepare(
      'INSERT INTO notifications (id, title, description, type, time, read) VALUES (?, ?, ?, ?, ?, ?)'
    );
    for (const n of data.notifications) insertNotif.run(n.id, n.title, n.description, n.type, n.time, n.read ? 1 : 0);
    ...
    for (const e of data.employees) insertEmp.run(e.id, e.name, e.role, e.branchId, e.email, e.status);
    ...
    for (const t of data.tasks) insertTask.run(t.id, t.title, t.description, t.priority, t.status, t.assignedTo, t.dueDate);
    ...
    for (const m of data.supportMessages) insertSupport.run(m.id, m.role, m.text, m.time);
    ...
    for (const i of data.integrations) insertInt.run(i.id, i.name, i.description, i.category, i.icon, i.connected ? 1 : 0, i.webhookUrl ?? null);
